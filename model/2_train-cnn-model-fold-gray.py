...
parser.add_argument("--dsprefix", help="folder and csv common name", default="utkface-wild-pepper")
...
# PATHS !!
ds_path = os.path.join(root_path, ds_prefix+'/')
csv_path = os.path.join(root_path, ds_prefix+'.csv')
...
for f, (train_index, val_index) in enumerate(kfold.split(training_data, training_data['age-group'])):
    print(f"Fold {f}:")
    ...
    val_data = training_data.iloc[val_index]
    ...
    print(history)
    model.save(results_folder+"/model_"+str(f))
    ...
    plt.grid(True)
    plt.xlabel('epoch')
    plt.savefig(results_folder+"/metrics_"+str(f)+".jpg")
	
    # LOAD BEST MODEL to evaluate the performance of the model
    # https://medium.com/the-owl/k-fold-cross-validation-in-keras-3ec4a3a00538
    model.load_weights(results_folder+'/model_'+str(f)+'.h5')
...
test_data.astype({'gender': 'int', 'age': 'int'})
...
recall.update_state(test_data["gender"], y_pred_gender)
print("Recall on gender: ", recall.result().numpy())
#recall.update_state(test_data["age"], y_pred_age)
#print("Recall on age: ", recall.result().numpy())

# Confusion matrix GENDER
cm = confusion_matrix(test_data['gender'], y_pred_gender, labels=np.unique(y_pred_gender))
...
cm = confusion_matrix(test_data["age-group"], y_pred_groups)
...
indices = random.sample(np.arange(0,len(test_data.index)).tolist(),9)
